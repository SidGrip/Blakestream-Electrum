...
class SessionManager:
    ...
    async def limited_history(self, hashX):
        ...
        try:
            ...
            # Do not cache empty histories.  Wallets commonly poll fresh
            # addresses before funding them; if that empty result is cached and
            # block notifications miss the hashX, confirmed history can remain
            # invisible until ElectrumX restarts even though UTXOs are indexed.
            if result:
    ...
    async def _notify_sessions(self, height, touched):
        ...
        if height_changed:
            # Confirmed history can change for hashXs that were queried before
            # their first confirmed transaction.  If mempool tracking missed the
            # tx or the touched set is incomplete, touched-only invalidation can
            # leave stale empty histories until process restart.
...
class ElectrumX(SessionBase):
    ...
    async def address_status(self, hashX):
        ...
        confirmed_tx_hashes = {tx_hash for tx_hash, height in db_history}
        mempool = await self.mempool.transaction_summaries(
            hashX, exclude_tx_hashes=confirmed_tx_hashes)
    ...
    async def hashX_listunspent(self, hashX):
        ...
        utxos = sorted(utxos)
        confirmed_outpoints = {(utxo.tx_hash, utxo.tx_pos) for utxo in utxos}
        history, cost = await self.session_mgr.limited_history(hashX)
        confirmed_tx_hashes = {tx_hash for tx_hash, height in history}
        mempool_utxos = await self.mempool.unordered_UTXOs(
            hashX, exclude_tx_hashes=confirmed_tx_hashes)
        utxos.extend(utxo for utxo in mempool_utxos
                     if (utxo.tx_hash, utxo.tx_pos) not in confirmed_outpoints)
        self.bump_cost(cost + 1.0 + len(utxos) / 50)
        spends = await self.mempool.potential_spends(
            hashX, exclude_tx_hashes=confirmed_tx_hashes)
    ...
    async def get_balance(self, hashX):
        ...
        history, cost = await self.session_mgr.limited_history(hashX)
        confirmed_tx_hashes = {tx_hash for tx_hash, height in history}
        unconfirmed = await self.mempool.balance_delta(
            hashX, exclude_tx_hashes=confirmed_tx_hashes)
        self.bump_cost(cost + 1.0 + len(utxos) / 50)
    ...
    async def unconfirmed_history(self, hashX, *, exclude_tx_hashes=frozenset()):
        # Note unconfirmed history is unordered in electrum-server
        # height is -1 if it has unconfirmed inputs, otherwise 0
        result = [{'tx_hash': hash_to_hex_str(tx.hash),
                   'height': -tx.has_unconfirmed_inputs,
                   'fee': tx.fee}
                  for tx in await self.mempool.transaction_summaries(
                      hashX, exclude_tx_hashes=exclude_tx_hashes)]
    ...
    async def confirmed_and_unconfirmed_history(self, hashX):
        ...
        return conf + await self.unconfirmed_history(
            hashX, exclude_tx_hashes=confirmed_tx_hashes)
    ...
    async def scripthash_get_mempool(self, scripthash):
        ...
        self.bump_cost(cost)
        confirmed_tx_hashes = {tx_hash for tx_hash, height in history}
        return await self.unconfirmed_history(
            hashX, exclude_tx_hashes=confirmed_tx_hashes)
