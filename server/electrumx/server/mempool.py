...
class MemPool(object):
    ...
    async def balance_delta(self, hashX, *, exclude_tx_hashes=frozenset()):
    ...
    async def potential_spends(self, hashX, *, exclude_tx_hashes=frozenset()):
        ...
        for tx_hash in self.hashXs.get(hashX, ()):
            if tx_hash in exclude_tx_hashes:
                continue
            tx = self.txs[tx_hash]
            result.update(tx.prevouts)
        return result

    async def transaction_summaries(self, hashX, *, exclude_tx_hashes=frozenset()):
    ...
    async def unordered_UTXOs(self, hashX, *, exclude_tx_hashes=frozenset()):
