...
class ServerBase:
    ...
    async def run(self):
        ...
        # Newer aiorpcx runtimes removed the old report_crash keyword from
        # spawn(); this tree already has its own exception handler and explicit
        # shutdown flow, so the plain spawn() call is the compatible path.
