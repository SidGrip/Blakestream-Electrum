...
class TestCommands(ElectrumTestCase):
    ...
    async def test_getblockhash(self):
        self.assertEqual('int', electrum.commands.known_commands['getblockhash'].arg_types['height'])
        self.assertEqual('Block height', electrum.commands.known_commands['getblockhash'].arg_descriptions['height'])

        chain = mock.Mock()
        chain.height.return_value = 1000
        chain.get_hash.side_effect = lambda height: {
            0: '01' * 32,
            1000: '02' * 32,
        }[height]
        network = mock.Mock()
        network.blockchain.return_value = chain
        cmds = Commands(config=self.config, network=network)

        self.assertEqual('01' * 32, await cmds.getblockhash(0))
        self.assertEqual('02' * 32, await cmds.getblockhash("1000"))
        with self.assertRaises(UserFacingException):
            await cmds.getblockhash(-1)
        with self.assertRaises(UserFacingException):
            await cmds.getblockhash(1001)
