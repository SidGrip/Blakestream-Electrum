...
class TestLNUtil(ElectrumTestCase):
    @staticmethod
    def _uses_blakecoin_network() -> bool:
        return constants.net.SEGWIT_HRP in {"blc", "tblc", "rblc"}

    def assertHtlcTxMatchesVector(
            self, *, ref_raw: str, htlc: bytes, htlc_output_index: int,
            amount_msat: int, htlc_payment_preimage: bytes, remote_htlc_sig: str,
            success: bool, cltv_abs: int, local_feerate_per_kw: int,
            our_commit_tx: PartialTransaction, has_anchors: bool) -> None:
        our_raw = self.htlc_tx(
            htlc=htlc,
        ...
        if not self._uses_blakecoin_network():
            self.assertEqual(ref_raw, our_raw)
            return

        # Blakecoin keeps Bitcoin-style BIP143 witness sighashes but uses a
        # Blakecoin txid hash. BOLT03's exact Bitcoin HTLC-resolution tx hex
        # therefore diverges at the prevout txid and signatures. Keep the
        # script, value, locktime, sequence, sighash flags, and witness-shape
        # checks exact for the active Blakecoin network.
        our_tx = Transaction(our_raw)
        ref_tx = Transaction(ref_raw)
        self.assertEqual(ref_tx.version, our_tx.version)
        self.assertEqual(ref_tx.locktime, our_tx.locktime)
        self.assertEqual(ref_tx.outputs(), our_tx.outputs())
        self.assertEqual(1, len(our_tx.inputs()))
        self.assertEqual(1, len(our_tx.outputs()))

        our_txin = our_tx.inputs()[0]
        ref_txin = ref_tx.inputs()[0]
        self.assertEqual(ref_txin.prevout.out_idx, our_txin.prevout.out_idx)
        self.assertEqual(htlc_output_index, our_txin.prevout.out_idx)
        self.assertEqual(our_commit_tx.txid(), our_txin.prevout.txid.hex())
        self.assertEqual(ref_txin.nsequence, our_txin.nsequence)

        remote_sighash = Sighash.ALL
        if has_anchors:
            remote_sighash = Sighash.ANYONECANPAY | Sighash.SINGLE
        witness = our_txin.witness_elements()
        self.assertEqual(5, len(witness))
        self.assertEqual(b'', witness[0])
        self.assertEqual(bfh(remote_htlc_sig) + remote_sighash.to_bytes(1, 'big'), witness[1])
        self.assertGreater(len(witness[2]), 1)
        self.assertEqual(Sighash.to_sigbytes(Sighash.ALL), witness[2][-1:])
        self.assertEqual(htlc_payment_preimage if success else b'', witness[3])
        self.assertEqual(htlc, witness[4])
    ...
    def test_commitment_tx_with_all_five_HTLCs_untrimmed_minimum_feerate(self):
        ...
        for i in range(5):
            self.assertHtlcTxMatchesVector(
                ref_raw=output_htlc_tx[i][1],
                htlc=htlc[i],
                htlc_output_index=htlc_output_index[i],
                amount_msat=htlcs[i].htlc.amount_msat,
                htlc_payment_preimage=htlc_payment_preimage[i],
                remote_htlc_sig=signature_for_output_remote_htlc[i],
                success=output_htlc_tx[i][0],
                cltv_abs=htlc_cltv_timeout[i] if not output_htlc_tx[i][0] else 0,
                local_feerate_per_kw=local_feerate_per_kw,
                our_commit_tx=our_commit_tx,
                has_anchors=False,
            )
            ...
            with self.subTest(test_vector['Name']):
                ...
                if use_test_htlcs:
                    for output_index, (test_index, htlc) in enumerate(sorted_htlcs.items()):
                        ...
                        self.assertHtlcTxMatchesVector(
                            ref_raw=htlc_descs[output_index]['ResolutionTxHex'],
    ...
    def test_lnworker_decode_channel_update_msg(self):
        bitcoin_msg_without_prefix = bytes.fromhex("")
        msg_without_prefix = bitcoin_msg_without_prefix[:64] + constants.net.rev_genesis_bytes() + bitcoin_msg_without_prefix[96:]
        # good messages
        self.assertNotEqual(
            None,
        ...
        self.assertEqual(
            None,
            LNWallet._decode_channel_update_msg(bitcoin_msg_without_prefix))
